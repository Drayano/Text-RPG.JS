...
function save_game() {
    ...
    let current_time = yyyy + mm + dd + "-" + time;
    ...
    dlAnchorElem.setAttribute("download", `${player_profile[0].name}-TextRPG-${current_time}.json`);
    dlAnchorElem.click();
}
function load_game() {
    if (localStorage.getItem(`${player1.name}`) === null) {
        ...
        let item = localStorage.getItem(player1.name);
...
function player_setup() {
    let item1 = localStorage.getItem("Player Name");
    let item2 = localStorage.getItem("item1");
